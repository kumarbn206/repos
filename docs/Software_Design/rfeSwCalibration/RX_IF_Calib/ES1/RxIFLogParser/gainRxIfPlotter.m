function gainRxIfPlotter(dataPath,targetFileName,candidateFileName,dataToUse,legacyLog,indxInitFirsAvg,gainErrOutlierTol,rxIfTotalGainSpec_dB,showSelectedAvg,rfeConfigStruct,legendValues)

dataFileExtension = '.txt';
%% Import data
if legacyLog
    [rxIfCalib_target,~] = importGainRxIFLogs([dataPath targetFileName dataFileExtension ],legacyLog);
    [rxIfCalib_candidate,~]= importGainRxIFLogs([dataPath candidateFileName dataFileExtension],legacyLog);
else
...
%Conversion of +/- total Rx Gain specification (in dB) into percentage is carried out via Err = ((rxIfTotalGainSpec_dB-(-1*rxIfTotalGainSpec_dB))/configGain ) *100 [%] Maximum percentage error associated to +-2dB error in the configured gain
plotMaxError = ((rxIfTotalGainSpec_dB-(-1*rxIfTotalGainSpec_dB))/configGain) * 100; 
...
% if(useRC0_8_12)
%     legendValues.targetLegend = 'RC 0.8.12';
% end
...
pHandlerSubplot(1) = subplot(3,1,1);hold on;grid on;
            ...
            %     stem(rxIfCalib_candidate(indexOutlier_candidate),'Color',[0.4940 0.1840 0.5560]);
                hold off;
                title([replace(dataToUse,'_','-') ': Per Iteration IF Gain relative error']);
                legend({[ legendValues.targetLegend '_{rms:' num2str(target_relErrorRms) '}'],[ legendValues.candidateLegend '_{rms:' num2str(candidate_relErrorRms) '}']});
                xlabel("Iteration");
...
pHandlerSubplot(2) = subplot(3,1,2);hold on;grid on;
                ...
                hold off;
                title([replace(dataToUse,'_','-') ': All Rxs averaged, IF Gain relative error']);
                legend({[ legendValues.targetLegend '_{rms:' num2str(avg_target_relErrorRms) '}'],[ legendValues.candidateLegend '_{rms:' num2str(avg_candidate_relErrorRms) '}']});
                xlabel("Iteration");
                ylabel("Relative Error [%]");
...
pHandlerSubplot(3) = subplot(3,1,3);hold on;grid on;
    ...
    title([replace(dataToUse,'_','-') ': Histogram All Rxs averaged, IF Gain relative error']);
    legend({legendValues.targetLegend,[ legendValues.targetLegend ' distfit'], legendValues.candidateLegend, [legendValues.candidateLegend ' distfit']});
    ylabel("Occurrences");
